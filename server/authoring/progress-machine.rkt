...
(define-type State (U MustSubmitNext MustReviewNext Finished))
(struct: MustSubmitNext ([student : User] [course : Course] [assignment : Assignment] [step : Step]))
(struct: MustReviewNext ([student : User] [course : Course] [assignment : Assignment] [step : Step] [reviewee : (Listof User)]))
(struct: Finished ())
...
(define-type Input (U SubmitStep SubmitReview))
