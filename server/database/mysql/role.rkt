...
(provide table class-id class-id-type user-id user-id-type role)
(define table "role")
...
(define (select-role sql-conn class uid)
  (let ((query (prepare sql-conn (merge "SELECT" role "FROM" table "WHERE" class-id "=? AND" user-id "=? LIMIT 1"))))
...
(define (select-users-in-class sql-conn class s-role limit page)
  (let* ((lower (* limit page))
         (upper (+ lower limit))
         (query (merge "SELECT" user-id "FROM" table "WHERE" class-id "=? AND" role "=? LIMIT ?, ?"))
